...
from graph import draw_single_graph
...
For section > 1 it finds the best-performing model from the previous oracle 
and passes it as an input parameter for the next run.

Functions:
----------
- run_oracle(algorithm, model_previous_step, section, learning_timesteps, env_name, env_seed, policy_seed, noise_seed, buffer_seed, parameter, seed_range):
    Executes the 'oracle.py' script with the provided parameters using subprocess.run.
    Includes the best model from the previous step for if section > 1.

- main(algorithm, sections_number, learning_timesteps, env_name, env_seed, policy_seed, noise_seed, buffer_seed, parameter, seed_range):
    Divides the total learning timesteps into sections and iteratively runs the oracle script for each section.
    After section > 1 it finds and sets the best model from the previous section to be used in the next section.

Usage:
------
To run the script from the command line, use the following command:
    python superOracle.py --algorithm {SAC} --sections_number SECTIONS_NUMBER --learning_timesteps LEARNING_TIMESTEPS
...
Concrete example : 
    python superOracle.py --algorithm SAC --sections_number 3 --learning_timesteps 30000 
